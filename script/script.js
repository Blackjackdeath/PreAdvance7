...
async function requestSearch(serch) {
  try {
    const response = await fetch(`http://www.omdbapi.com/${serch}&apikey=cc8acbe3`);
...
async function requestInfo(id) {
  try {
    const response = await fetch(`http://www.omdbapi.com/?i=${id}&plot=full&apikey=cc8acbe3`);
    const data = await response.json();
    writeInfo(data);
